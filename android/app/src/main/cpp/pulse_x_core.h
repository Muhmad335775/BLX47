...
PX_EXPORT int32_t px_core_get_live_voice_scene(PxVoiceSceneDescriptor* out_scene);
PX_EXPORT int32_t px_core_generate_voice_scene(int32_t device_tier);
PX_EXPORT int32_t px_core_get_voice_scene(PxVoiceSceneDescriptor* out_scene);
PX_EXPORT int32_t px_core_get_witch_special(PxWitchSpecialDescriptor* out_witch);

PX_EXPORT int32_t px_core_build_cinematic_render(int32_t device_tier);
PX_EXPORT int32_t px_core_get_cinematic_render(PxCinematicRenderDescriptor* out_render);

/* Paper 7 - PULSE-X BRAND + VIDEO OUTPUT API.
 * px_core_apply_watermark: valid during EXPORT state only, after
 * begin_export. Always sets enabled=1 - there is no argument or
 * code path that disables the watermark.
 * px_core_get_watermark_state / px_core_get_final_video_frame:
 * valid only after apply_watermark has run for this take. */
PX_EXPORT int32_t px_core_apply_watermark(void);
PX_EXPORT int32_t px_core_get_watermark_state(PxBrandWatermarkDescriptor* out_watermark);
PX_EXPORT int32_t px_core_get_final_video_frame(PxFinalVideoFrame* out_frame);
