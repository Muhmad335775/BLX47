...
void build_depth_layers(const PxVoiceSceneDescriptor* voice_scene,
    ...
    const float movement = voice_scene->mapping.movement_amount;
    const float visual = voice_scene->mapping.visual_response;
...
void build_camera(const PxVoiceSceneDescriptor* voice_scene,
    ...
    out_camera->field_of_view_deg = 45.0f + (voice_scene->mapping.effects_intensity * 20.0f);

    out_camera->pan_speed =
        voice_scene->mapping.animation_timing_scale * 0.6f;
    out_camera->tilt_speed =
        voice_scene->mapping.visual_response * 0.4f;
    out_camera->dolly_speed =
        voice_scene->mapping.scale_motion_lighting * 0.5f;
}
...
void build_lighting(const PxWorldDescriptor* world,
    ...
    out_lighting->key_light_intensity = 0.5f + (world->atmosphere_intensity * 0.5f);
    out_lighting->fill_light_intensity = 0.3f + (world->lighting_warmth * 0.3f);
    out_lighting->rim_light_intensity = world->atmosphere_intensity * 0.4f;
    out_lighting->color_temperature_shift = (world->lighting_warmth - 0.5f) * 2.0f;
}
...
void build_render_descriptor(const PxVoiceSceneDescriptor* voice_scene,
    ...
    out_descriptor->fx_flags = select_fx_flags(
        static_cast<PxScenarioFamily>(voice_scene->scenario_family),
        world->has_fireworks);

    out_descriptor->atmosphere_density = world->atmosphere_intensity;
