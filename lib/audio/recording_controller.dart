...
class RecordingController {
  ...
  Future<void> onPressStart() async {
    ...
    try {
      ...
      if (!_core.beginRecording()) {
        lastErrorMessage =
            'core.beginRecording() returned false, state=${_core.state}';
  ...
  /// Ends the recording and leaves CORE in GENERATION state.
  /// It is the caller's responsibility (e.g. after a successful
  /// world-generation step) to eventually call [returnCoreToReady].
  /// This keeps CORE in GENERATION long enough for Paper 4's
  /// px_core_generate_world() to be called validly.
  Future<void> _finishRecording() async {
    ...
    try {
      ...
      if (!endedOk || analysis == null) {
        lastErrorMessage =
            'endRecording=$endedOk analysis=${analysis == null ? "null" : "ok"} state=${_core.state}';
        _phaseController.add(RecordingPhase.error);
        return;
      }
  ...
  /// Call this explicitly once all GENERATION-state work (e.g.
  /// world generation, render, export) for this take is done.
  void returnCoreToReady() {
    if (_core.state == PxState.export_ || _core.state == PxState.generation) {
      _core.returnToReady();
    }
  }
