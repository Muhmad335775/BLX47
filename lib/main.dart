...
class _RecordingPageState extends State<RecordingPage>
  ...
  void _runWorldCheck() {
    try {
      ...
      final landmark = world.getDailyLandmark(
        daySeed: DateTime.now().day,
      );

      // World-generation work is done - now it's safe to return
      // CORE from GENERATION back to READY for the next take.
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('BLX47 - Paper 4')),
      body: Stack(
        children: [
          Padding(
