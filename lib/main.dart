...
import 'ffi/pulse_x_brand.dart';
import 'video/video_export.dart';
...
class _RecordingPageState extends State<RecordingPage>
  ...
  void initState() {
    ...
    _controller.phaseStream.listen((phase) {
      ...
      if (phase == RecordingPhase.ready) {
        _runFullPipelineCheck();
      }
    });
  }

  Future<void> _runFullPipelineCheck() async {
    _runWorldCheck();
    _runVoiceSceneCheck();
    _runRenderCheck();
    await _runBrandCheck();
  ...
  void _runRenderCheck() {
    try {
      ...
      final renderStarted = core.beginRender();
      if (!renderStarted) {
        setState(() {
          _renderDebugText =
              'RENDER ERROR: beginRender failed, state=${core.state}';
        });
        return;
      }
  ...
  Future<void> _runBrandCheck() async {
    try {
      final core = PulseXCore();
      final exportStarted = core.beginExport();
      if (!exportStarted) {
        setState(() {
          _brandDebugText =
              'BRAND ERROR: beginExport failed, state=${core.state}';
        });
        return;
      }

      final brand = PulseXBrand();
      final applied = brand.applyWatermark();
      final watermark = brand.getWatermarkState();
      final videoFrame = brand.getFinalVideoFrame();

      if (!applied || watermark == null || videoFrame == null) {
        setState(() {
          _brandDebugText =
              'BRAND ERROR: applied=$applied watermark=$watermark videoFrame=$videoFrame';
        });
        return;
      }

      final durationSeconds =
          (videoFrame.durationMs / 1000).clamp(1, 5).round();

      final path = await VideoExport.instance.encodeSolidColorVideo(
      ...
      setState(() {
        _brandDebugText =
            'Brand OK - watermark: ${watermark.enabled} - '
            'witchGlow: ${watermark.witchGlowIntensity.toStringAsFixed(2)} - '
            'video: ${videoFrame.width}x${videoFrame.height}@${videoFrame.fps} - '
            'durationMs: ${videoFrame.durationMs} - '
            'file: ${path ?? "ENCODE FAILED"}';
      });
    } catch (e, st) {
      setState(() {
        _brandDebugText = 'BRAND EXCEPTION: $e\n$st';
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('BLX47 - Paper 7')),
      body: Stack(
        children: [
