...
/* All engine memory is pre-allocated here at process load time.
 * No malloc/new/free/delete is performed inside RECORDING,
 * PROCESSING, GENERATION, or RENDER. */
...
int32_t px_core_begin_recording(void) {
    ...
    g_audio_analysis.sample_count = 0;
...
int32_t px_core_feed_audio(const float* samples, int32_t sample_count) {
    ...
    g_audio_analysis.rms_level = sum_sq > 0.0f
        ? static_cast<float>(sum_sq / static_cast<float>(sample_count))
        : 0.0f;
    g_audio_analysis.peak_level = peak;
    g_audio_analysis.dominant_frequency_hz = 0.0f;
    g_audio_analysis.sample_count += sample_count;
...
int32_t px_core_end_recording(void) {
    ...
    g_scene_descriptor.energy = g_audio_analysis.rms_level;
    g_scene_descriptor.tempo_bpm = 0.0f;
    g_scene_descriptor.scene_id = 0;
